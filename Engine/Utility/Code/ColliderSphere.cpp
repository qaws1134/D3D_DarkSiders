...
//매시용 
HRESULT CColliderSphere::Ready_Collider(const _vec3 * pPos, _ulong dwVtxCnt)
{
	D3DXComputeBoundingSphere(pPos, dwVtxCnt, sizeof(_vec3), &m_vCenter, &m_fRadius);
...
CColliderSphere * CColliderSphere::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3* pPos, _ulong dwVtxCnt)
{
	CColliderSphere*	pInstance = new	CColliderSphere(pGraphicDev);

	if (FAILED(pInstance->Ready_Collider(pPos,dwVtxCnt)))
		Safe_Release(pInstance);
