...
{
	m_RenderGroup[RENDER_UI].sort(Compare_WorldZ);
...
void CRenderer::Render_MFCGameObject(LPDIRECT3DDEVICE9 & pGraphicDev)
{
	Render_Priority(pGraphicDev);
...
void Engine::CRenderer::Render_Blend(LPDIRECT3DDEVICE9& pGraphicDev)
{
	...
	Get_RenderTargetTexture(pEffect, L"Target_Specular", "g_SpecularTexture");
