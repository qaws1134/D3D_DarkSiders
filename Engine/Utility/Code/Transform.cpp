...
void CTransform::Get_WorldMatrix(_matrix * pMatrix) 
{
	*pMatrix = m_matWorld;
}

void Engine::CTransform::Set_Scale( _float fX,  _float fY,  _float fZ)
{
	m_vScale.x = fX;
	m_vScale.y = fY;
	m_vScale.z = fZ;
...
void CTransform::Set_Rot( _vec3 * vAngle)
{
	m_vAngle = *vAngle;
}

void CTransform::Set_Rot( _float & fX,  _float & fY,  _float & fZ)
{
	m_vAngle.x = fX;
	m_vAngle.y = fY;
	m_vAngle.z = fZ;
...
void Engine::CTransform::Set_WorldMatrix( _matrix* pWorld)
{
	m_matWorld = *pWorld;
}

void Engine::CTransform::Move_Pos( _vec3* pDir,  _float& fSpeed,  _float& fTimeDelta)
{
	m_vInfo[INFO_POS] += *pDir * fSpeed * fTimeDelta;
}

void Engine::CTransform::Rotation(ROTATION eType,  _float& fAngle)
{
